...
describe Delivery do
  ...
  describe "next_10" do
    ...
    it "should return the next ten delivery dates for this supplier" do
      Delivery.next_10_dates(1).should have(10).dates
    end

    it "should return them with the next first" do
      Delivery.next_10_dates(1).first[0].should == Date.new(2210, 4, 16)
    end

    describe "if there are multiple rounds on a delviery day" do
      it "should only show the delivery date once" do
        Delivery.next_10_dates(1).find_all{ |d| d[0] == Date.new(2210,4,16)}.should have(1).delivery
      end

      it "should contain the round_ids for the date" do
        (Delivery.next_10_dates(1).find_all{ |d| d[0] == Date.new(2210,4,16)}
